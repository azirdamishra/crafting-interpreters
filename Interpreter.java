...
public class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void>{
    ...
    private Environment environment = globals; //changes as we enter and exit local scopes, it tracks the current env
    ...
    public Void visitClassStmt(Stmt.Class stmt){ //interpret the class declaration
        ...
        LoxClass klass = new LoxClass(stmt.name.lexeme, (LoxClass)superclass, methods); //runtime representation of a class
    ...
    public Void visitFunctionStmt(Stmt.Function stmt){
        LoxFunction function = new LoxFunction(stmt, environment, false); //this is the env that is active when the functionis declared not when it is called
    ...
    public Object visitAssignExpr(Expr.Assign expr) {
       ...
       if(distance != null){
        ...
        //if the var's scope distance is not found, we assume it to be global and handle it the same way as before
        globals.assign(expr.name, value); 
       }
        
       return value;
        /*
        assignment is an expression that can be nested inside other expressiond
        var a = 1;
        print a = 2; //"2" is printed
         */
    }
    ...
    private Object lookUpVariable(Token name, Expr expr){
        Integer distance = locals.get(expr); //looking up the resolved distance in the map (locals only, globals are not in the map)
        if(distance != null){
